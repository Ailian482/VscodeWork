...
-- 国内机票直加产品表添加适用机构类型集合字段
call AddColumnUnlessExists('flight_wrapper_product', 'corp_types', 'varchar(300) not null comment "适用机构类型集合"');

-- 初始化历史数据的适用机构类型集合
update flight_wrapper_product set corp_types = 'Corporation,Government,GovernmentSK,SK' where LENGTH(corp_types) = 0;
```
## 