...
# 2024-12-11
## 自主预订火车票/国内酒店发送预警通知到企微群（测试中：12.12-）
- 后端分支: tmc:pref-orderBookingRemind3
```
新增火车票散客预定通知场景， 主要5个场景 申请出票失败/改签申请失败/ 预定单自动出票失败/ 改签自动出票失败/ 退票失败 ， 前两个场景一般不会发生，除非相应资源服务异常了，暂时可以不用管
新增国内酒店散客预定通知场景， 主要4个场景 ，自动订房失败/ 自动订房超时转人工/ 自动退房失败/ 部分退订
```
