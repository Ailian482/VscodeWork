...
class TestCancelFlightOrder(unittest.TestCase):

    @classmethod
    def setUpClass(cls) -> None:
        cls.logger = Log.log("test_login").logger
        cls.operateExcel = operationExcel.operationExcel(readconfig().get_table_name("flight"), "cancel")
        cls.url = readconfig().get_flight_url("cancel")
    ...
    def test_cancel_case01(self):
        """正常取消"""
        self.logger.info('国内机票订单取消--用例01--正常取消')
        self.response = FlightProcess().cancel_flight()
    ...
    def test_cancel_case02(self):
        """订单号-空"""
        self.logger.info('国内机票订单取消--用例02--订单号为空')
    ...
    def test_cancel_case03(self):
        """订单号-不存在"""
        self.logger.info('国内机票订单取消--用例03--订单号不存在')
    ...
    def test_cancel_case04(self):
        """订单号-非数字"""
        self.logger.info('国内机票订单取消--用例04--订单号非数字')
        order_id = "aaaa"
        self.url = "{0}/{1}".format(self.url, order_id)
        self.response = UrlHeader().urlHeader(self.url, self.headers)
        self.get_error(self.response)
    ...
    def test_cancel_case05(self):
        """订单号-不可取消"""
        self.logger.info('国内机票订单取消--用例05--订单号不可取消')
