...
class TestReturnFlight(unittest.TestCase):
    ...
    def setUpClass(cls) -> None:
        ...
        cls.url = readconfig().get_flight_url("cancelPnr")
        cls.operateExcel = operationExcel.operationExcel(readconfig().get_table_name("flight"), "cancelPnr")
    ...
    def test_return_case01(self):
        """正常取消"""
        self.logger.info('国内机票取消pnr--用例01--正常取消')
        task_id, order_id, ticket_ids, header = FlightProcess().get_id("cancelPnr")
        if not task_id:
    ...
    def test_return_case02(self):
        """任务id--空"""
        self.logger.info('国内机票取消pnr--用例02--任务id-空')
        task_id = ""
        url = "{0}{1}".format(self.url, task_id)
        ...
        self.assertIn("请传入正确的任务ID", str(self.response.json()["message"]))
        self.test_result = "PASS"

    # 用例3--任务id--不存在
    def test_return_case03(self):
        """任务id--不存在"""
        self.logger.info('国内机票取消pnr--用例03--任务id--不存在')
    ...
    def test_return_case04(self):
        """任务id--已取消"""
        self.logger.info('国内机票取消pnr--用例04--任务id--已取消')
        task_id = 122725328106950656
        url = "{0}{1}".format(self.url, task_id)
        self.response = self.api.urlHeader(url, self.headers)
        self.get_error(self.response)
        self.assertEqual(self.response_code, 1, "返回的code不是1，是{0}，错误内容：{1}".format(self.response_code, self.message))
        self.assertIn("当前PNR取消任务状态[taskStatus: Completed]不可操作", str(self.response.json()["message"]))
        self.test_result = "PASS"
