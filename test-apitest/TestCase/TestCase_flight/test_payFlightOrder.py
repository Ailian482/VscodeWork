...
class TestPayFlightOrder(unittest.TestCase):

    @classmethod
    def setUpClass(cls) -> None:
        cls.logger = Log.log("test_login").logger
        cls.operateExcel = operationExcel.operationExcel(readconfig().get_table_name("flight"), "pay")
        cls.url = readconfig().get_flight_url("pay")
    ...
    def test_pay_case01(self):
        """正常支付"""
        self.logger.info('国内机票订单支付--用例01--正常支付')
        self.response, order_id, header = FlightProcess().pay_flight()
        ...
        self.assertIn("OK", self.response["message"])
        self.test_result = "PASS"

    # 用例2--订单号-空
    def test_pay_case02(self):
        """订单号-空"""
        self.logger.info('国内机票订单支付--用例02--订单号为空')
        ...
        self.assertIn("订单id不能为空", self.response.json()["message"])
        self.test_result = "PASS"
        # self.assertEqual(response_code, 99, "返回的code不是99，是{0}".format(response_code))
        # self.assertIn("For input string", self.response.json()["message"])
        self.test_result = "PASS"

    # 用例3--订单号-不存在
    def test_pay_case03(self):
        """订单号-不存在"""
        self.logger.info('国内机票订单支付--用例03--订单号不存在')
    ...
    def test_pay_case04(self):
        """订单号--非数字"""
        self.logger.info('国内机票订单支付--用例04--订单号非数字')
        order_id = "aaaa"
        self.url = "{0}{1}".format(self.url, order_id)
        self.response = UrlHeader().urlHeader(self.url, self.headers)
        self.get_error(self.response)
    ...
    def test_pay_case05(self):
        """订单号-不可支付"""
        self.logger.info('国内机票订单支付--用例05--订单号不可支付')
        order_id = "138259946956525555"
        self.url = "{0}{1}".format(self.url, order_id)
        self.response = UrlHeader().urlHeader(self.url, self.headers)
        response_code = self.response.json()["code"]
        self.get_error(self.response)
        self.assertEqual(self.response_code, 99, "返回的code不是99，是{0}，错误内容：{1}".format(self.response_code, self.message))
        self.assertIn("查找的记录[id: 138259946956525555]不存在", self.response.json()["message"])
        # self.assertEqual(response_code, 2, "返回的code不是2，是{0}".format(response_code))
        # self.assertIn("当前员工没有权限操作此订单", self.response.json()["message"])
        self.test_result = "PASS"
