...
class TestReturnFlight(unittest.TestCase):
    ...
    def setUpClass(cls) -> None:
        ...
        cls.url = readconfig().get_flight_url("returnCancel")
        cls.operateExcel = operationExcel.operationExcel(readconfig().get_table_name("flight"), "returnCancel")
    ...
    def test_return_case01(self):
        """正常取消"""
        self.logger.info('国内机票退票取消--用例01--正常取消')
        task_id, order_id, ticket_ids, header = FlightProcess().get_id("ReturnConfirm")
        if not task_id:
    ...
    def test_return_case02(self):
        """任务id--空"""
        self.logger.info('国内机票退票取消--用例02--任务id-空')
        task_id = ""
        url = "{0}{1}".format(self.url, task_id)
    ...
    def test_return_case03(self):
        """任务id--不存在"""
        self.logger.info('国内机票退票取消--用例03--任务id--不存在')
    ...
    def test_return_case04(self):
        """任务id--已取消"""
        self.logger.info('国内机票退票取消--用例04--任务id--已取消')
        task_id = 124934963060674560
        url = "{0}{1}".format(self.url, task_id)
        self.response = self.api.urlHeader(url, self.headers)
        self.get_error(self.response)
        self.assertEqual(self.response_code, 99, "返回的code不是99，是{0}，错误内容：{1}".format(self.response_code, self.message))
        self.assertIn("不是申请退票状态", str(self.response.json()["message"]))
        self.test_result = "PASS"
