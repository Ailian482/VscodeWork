...
class HotelPreBill(unittest.TestCase):
    ...
    def setUpClass(cls) -> None:
        ...
        cls.url = "/front/v1/hotel/cancelOrder" + "?" + "orderId="
    ...
    def tearDown(self):
        ...
        self.operateExcel = operationExcel.operationExcel("hotel_interface.xls", "cancelOrder")
    ...
    # @unittest.skipIf(status=='200', "产生数据：用例1--未付款订单取消")
    # 用例1-- 订单取消（未付款状态）
    def test_search_case01(self):
        """订单取消 --未付款状态"""
        self.Test_name = '用例1 --订单取消 --未付款状态'
    ...
    # @unittest.skipIf(status=='200', "产生数据：用例2--已付款订单取消")
    # 用例2-- 订单取消（已付款状态）
    def test_search_case02(self):
        """订单取消 --已付款状态"""
        self.Test_name = '用例2 --订单取消 --已付款状态'
        self.logger.info(self.Test_name)
    ...
    def test_search_case05(self):
        """订单取消 --订单号不存在"""
        self.Test_name = '用例5 --订单取消 --订单号不存在'
        self.logger.info(self.Test_name)
    ...
    # 用例6 -- 订单号存在，但是不属于该登录账号 (该用例暂不考虑)
    # def test_search_case06(self):
    #     """订单取消 --订单号存在，但是不属于该登录账号"""
    #     self.Test_name = '用例6 --订单取消 --订单号存在，但是不属于该登录账号'
    #     self.logger.info(self.Test_name)
    #     url = self.url + "164041224267894784"
    #     response = self.Send.urlHeader(url, self.header)
    #     self.response = response.json()
    #     self.expected_result = "code = 99 / 没有权限访问"
    #     # 断言
    #     self.assertEqual(response.json()["code"], 99)
    #     self.assertIn("不存在", str(response.json()["message"]))
    #     self.test_result = "Pass"
