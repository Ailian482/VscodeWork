...
class HotelSearch(unittest.TestCase):
    ...
    def setUpClass(cls) -> None:
        cls.logger = Log.log().logger
        cls.jutil = JsonPathUtil.jsonpath_util()
        cls.data = JsonUtil.OperetionJson("Hotel_Customer_Json.json").get_data("Hotel_search")
        cls.url = "/front/v1/hotel/search"
    ...
    def test_search_case01(self):
        ...
        self.assertIn("深圳", str(response.json()["data"]))
        self.test_result = "Pass"

    # 用例2 -- 登录态校验
    def test_search_case02(self):
        """酒店查询-登录态校验"""
        self.Test_name = '用例2 --酒店查询 --登录态校验'
        self.logger.info(self.Test_name)
        self.data["checkInDate"] = (date.today()).strftime("%Y-%m-%d")
    ...
    def test_search_case05(self):
        """酒店查询 --入住日期大于离店日期"""
        self.Test_name = '用例5 --酒店查询 --入住日期大于离店日期'
        self.logger.info(self.Test_name)
        self.data["checkInDate"] = (date.today()).strftime("%Y-%m-%d")
        ...
        # 断言
        self.assertEqual(response.json()["code"], 0, "返回的code不是0，是{0}".format(response.json()["code"]))
        self.assertIn("0", str(response.json()["data"]["totalCount"]))
        self.test_result = "Pass"

    # 用例6 -- 入离住日期相同
    def test_search_case06(self):
        """酒店查询 --入离住日期相同"""
        self.Test_name = '用例6 --酒店查询 --入离住日期相同'
        self.logger.info(self.Test_name)
        self.data["checkInDate"] = (date.today()).strftime("%Y-%m-%d")
    ...
    def test_search_case07(self):
        """酒店查询 --入离日期为空"""
        self.Test_name = '用例7 --酒店查询 --入离日期为空'
        self.logger.info(self.Test_name)
    ...
    def test_search_case08(self):
        """酒店查询 --城市信息为空"""
        self.Test_name = '用例8 --酒店查询 --城市信息为空'
        self.logger.info(self.Test_name)
        self.data["checkInDate"] = (date.today()).strftime("%Y-%m-%d")
        self.data["checkOutDate"] = (date.today() + timedelta(days=+1)).strftime("%Y-%m-%d")
        self.data["cityCode"] = ""
