...
class TestReturnFlight(unittest.TestCase):
    ...
    def setUpClass(cls) -> None:
        ...
        cls.url = readconfig().get_intflight_url("cancelTicketConfirm")
        cls.operateExcel = operationExcel.operationExcel(readconfig().get_table_name("intflight"), "cancelTicketConfirm")
    ...
    def test_return_case01(self):
        """正常取消"""
        self.logger.info('国际机票取消出票确认--用例01--正常取消')
        task_id, order_id, header = IntflightProcess().get_id("TicketConfirm")
        if not task_id:
    ...
    def test_return_case02(self):
        """任务id-空"""
        self.logger.info('国际机票取消出票确认--用例02--任务id-空')
        task_id = ""
        url = "{0}{1}".format(self.url, task_id)
    ...
    def test_return_case03(self):
        """任务id--不存在"""
        self.logger.info('国际机票取消出票确认--用例03--任务id--不存在')
    ...
    def test_return_case04(self):
        """任务id--已取消"""
        self.logger.info('国际机票取消出票确认--用例04--任务id--已取消')
        task_id = 145831842606616576
        url = "{0}{1}".format(self.url, task_id)
        self.response = self.api.urlHeader(url, self.headers)
        self.get_error(self.response)
